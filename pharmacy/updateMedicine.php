...
<!-- bg-primary , bg-success , bg-warning , bg-info  and bg-danger  bg-light bg-dark -->
	...
	<div class="container-fluid form-color bod">
			...
			<div class="row">
				...
				<div class="col-sm-9">
					<form class="form-horizontal navbar-expand-md reg" method="post" action="updateMedicine.php" enctype="multipart/form-data">
...
<?php
	...
	if(isset($_POST['submit'])){
		...
		for($a = 0 ; $a < count($_POST['med_name']); $a++){
			//$sql = "INSERT INTO `medicine` (`med_name`,`company` , `quantity` , `d_o_m` , `d_o_e`) VALUES ('" . $_POST['med_name'][$a] ."' , '" . $_POST['company'][$a] ."' , '" . $_POST['quantity'][$a] ."' , '" . $_POST['d_o_m'][$a] . "' , '" . $_POST['d_o_e'][$a] ."') WHERE `med_name` = $_POST['med_name'][$a] ";
			$name = $_POST['med_name'][$a];
			$sql = "SELECT `quantity` FROM `medicine` WHERE `med_name` = '$name'";
			...
			if (!$run) {
				...
				$quan = $row[0];
				$newquan = $_POST['quantity'][$a] + $quan;
			    $dom = $_POST['d_o_m'][$a];
			    $doe = $_POST['d_o_e'][$a];
			    $qry ="UPDATE `medicine` SET `quantity`='$newquan',`d_o_m`='$dom',`d_o_e`='$doe' WHERE `med_name` = '$name'";
    			...
    			if($run == true){
    				...
    				?>
    				<script> alert('data not successfully updated');</script>
    				<?php
    			}
			}
		}
