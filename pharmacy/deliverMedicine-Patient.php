...
<!-- bg-primary , bg-success , bg-warning , bg-info  and bg-danger  bg-light bg-dark -->
	...
	<div class="container-fluid form-color bod">
	    <!--div>
	        <a href="#"><marquee style="color:red;">**Important information regarding shortage of medicine.</marquee></a>
	    </div-->
			<h1 align="center">Deliver Medicine to Patient</h1>
			<div class="row">
				<div class="col-sm-3"></div>
				<div class="col-sm-6">
				    
					<form class="form-horizontal navbar-expand-md reg" method="post" action="registration.php" enctype="multipart/form-data">
							...
							<div class="form-group row">
								<label class="control-label col-sm-4" for="fname">Visiting Id</label>
								<div class="col-sm-6">
									<input type="text" name="id" class="form-control" id="id" placeholder="Enter your visiting Id" STYLE="color: black; font-family: Verdana; font-weight: bold; font-size: 12px; background-color: #d6dfd6;">
								</div>
							</div>
...
<script type="text/javascript">
		...
		function addItem(){
			...
			var html  = "<tr>";
				html += "<td>" + items + "</td>";
				html +="<td><input name='med_name[]'></td>";
...
<?php
	...
	if(isset($_POST['submit'])){
		...
		for($a = 0 ; $a < count($_POST['med_name']); $a++){
		    $val = $_POST['med_name'][$a];
			$val1 = $_POST['quantity'][$a];
			$qry = "SELECT quantity FROM `medicine` WHERE `med_name` = '$val'";
			$result = mysqli_query($con ,$qry);
            $row = mysqli_fetch_assoc($result);
            $dbquantity = $row['quantity'];
            if($dbquantity >= $val1){
                $rslt = $dbquantity - $val1;
    		    if($a == 0){
    				/*$val = $_POST['med_name'][0];
    				$val1 = $_POST['quantity'][0];
    				echo $val;
    				echo $val1;*/
    				$sql = "UPDATE `prescribed_medicine` SET `s_id`='staff',`med_name`=  '$val' ,`quantity`= '$val1' WHERE `visit_id`= '$roll' and `quantity` = '0'";
    			//	mysqli_query($con,$sql);
    			}else{
        			$sql = "INSERT INTO `prescribed_medicine` (`visit_id`,`s_id` , `med_name` , `quantity`) VALUES ('$roll' , 'staff' , '" . $_POST['med_name'][$a] ."' , '" . $_POST['quantity'][$a] . "')";
    			}
    			$qry1 = "UPDATE `medicine` SET `quantity` = '$rslt' WHERE `med_name` = '$val'";
    			mysqli_query($con , $qry1);
    			mysqli_query($con,$sql);
            }else{
                ?>
                <script>alert('<?php echo ("Medicine is not available!")?>')</script>
                <?php
            }
		}
		?>
		<script type="text/javascript">alert("Data has Added Successfully!")</script>
		<?php
	}
